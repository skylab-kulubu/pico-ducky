            ...
            if line.startswith("IF"):
                ...
                if self.lastIfResult is False:
                    ...
                    if line.startswith("IF"):
                        ...
                        self.codeIter, self.lastIfResult = IF(
...
def evaluateExpression(expression):
    ...
    expression = re.sub(
        r"\$(\w+)", lambda m: str(variables.get(f"${m.group(1)}", 0)), expression
    )
...
def parseLine(line, script_lines):
    ...
    if line[:10] == "INJECT_MOD":
        ...
        if match:
            ...
            raise SyntaxError(
                f"Invalid variable update, declare variable first: {line}"
            )
        ...
        while evaluateExpression(condition) == True:
            ...
            while currentIterCode:
                ...
                )  # < very inefficient, should be replaced later.
        ...
        for _ in range(12):
            sendString(
                random.choice(letters + letters.upper() + numbers + specialChars)
            )
    elif line == "RESET":
        kbd.release_all()
...
async def blink_led(led):
    ...
    if board.board_id == "raspberry_pi_pico" or board.board_id == "raspberry_pi_pico2":
        ...
        or board.board_id == "raspberry_pi_pico2_w"
    ):
